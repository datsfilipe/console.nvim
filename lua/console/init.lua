...
local function run_interactive(cmdline, handler)
  ...
  os.remove(state.out_file)
  local f = io.open(state.cmd_file, 'w')
  if f then
    f:write(cmdline .. ' > ' .. state.out_file)
    f:flush()
    f:close()
  else
    vim.notify('Failed to write IPC command file', vim.log.levels.ERROR)
    return
  end

  local resume_group =
    api.nvim_create_augroup('ConsoleResume', { clear = true })
...
function M.run(cmdline)
  ...
  for key, handler in pairs(config.interactive) do
    if trimmed == key or trimmed:sub(1, #key + 1) == (key .. ' ') then
  ...
  if interactive_handler then
    if is_wrapper_session then
      vim.schedule(function()
        run_interactive(cmdline, interactive_handler)
      end)
      return
    else
      vim.notify(
        "Console wrapper not active. Suspend disabled for '" .. trimmed .. "'.",
        vim.log.levels.WARN
      )
    end
  end
  ...
  if config.hijack_bang then
    vim.keymap.set('c', '<CR>', function()
      ...
      if cmdtype == ':' and cmdline:match '^!' then
        local command_str = cmdline:sub(2)
        vim.fn.histadd('cmd', cmdline)
        if state.job then
          vim.schedule(function()
            state.keep_alive = true
            send_input(command_str)
